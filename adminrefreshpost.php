...
$sql1="SELECT * from adminuploads order by id desc";
$res1=$con->query($sql1);
while($row1=$res1 ->fetch_assoc())
{
  ...
  if($row1['data']!="")
    ...
    echo '<p>'.$row1['txtpost'].'</p>
      <button type="button" class="w3-button w3-theme-d1 w3-margin-bottom" onclick="acceptpost('.$id.' )"><i class="fa fa-thumbs-up"></i> '.$id.' Accept</button>
      <button type="button" class="w3-button w3-theme-d2 w3-margin-bottom" onclick="rejectpost('.$id.' )"><i class="fa fa-thumbs-down" ></i>  Reject</button>
    </div>';
}
 ?>
